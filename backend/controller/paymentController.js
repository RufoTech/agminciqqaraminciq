...
import generateOrderReceipt from "../utils/generateOrderReceipt.js";
...
// =====================================================================
// ÖDƏNIŞ SESSİYASI YARAT — createPaymentSession
// ---------------------------------------------------------------------
// POST /api/v1/products/create-payment-intent
// Body: { amount, currency, cardNumber, expiry, cvv }
//
// Simulyasiya modunda kart məlumatları əsas formatlara görə yoxlanılır.
// Uğurlu olduqda unikal paymentId qaytarılır.
// =====================================================================
export const createPaymentSession = catchAsyncErrors(async (req, res, next) => {
    const { amount, currency = "azn", cardNumber, expiry, cvv } = req.body;

    if (!amount || amount <= 0) {
        return next(new ErrorHandler("Ödəniş məbləği düzgün deyil", 400));
    }

    // ── KART VALİDASİYASI ─────────────────────────────────────────────
    if (!cardNumber || !expiry || !cvv) {
        return next(new ErrorHandler("Kart məlumatları tam daxil edilməyib", 400));
    }

    // Kart nömrəsi: yalnız rəqəmlər, 13–19 rəqəm (Luhn yoxlanılmır — simulyasiya)
    const cleanCard = cardNumber.replace(/\s+/g, "");
    if (!/^\d{13,19}$/.test(cleanCard)) {
        return next(new ErrorHandler("Kart nömrəsi düzgün deyil (13–19 rəqəm)", 400));
    }

    // Bitmə tarixi: MM/YY formatı, keçmiş olmamalıdır
    const expiryMatch = expiry.match(/^(\d{2})\/(\d{2})$/);
    if (!expiryMatch) {
        return next(new ErrorHandler("Bitmə tarixi MM/YY formatında olmalıdır", 400));
    }
    const expMonth = parseInt(expiryMatch[1], 10);
    const expYear  = parseInt("20" + expiryMatch[2], 10);
    if (expMonth < 1 || expMonth > 12) {
        return next(new ErrorHandler("Bitmə tarixi düzgün deyil", 400));
    }
    const now = new Date();
    if (expYear < now.getFullYear() || (expYear === now.getFullYear() && expMonth < now.getMonth() + 1)) {
        return next(new ErrorHandler("Kartın bitmə tarixi keçib", 400));
    }

    // CVV: 3–4 rəqəm
    if (!/^\d{3,4}$/.test(cvv)) {
        return next(new ErrorHandler("CVV 3 və ya 4 rəqəm olmalıdır", 400));
    }

    // ── SİMULYASİYA — unikal ID yarat ────────────────────────────────
    const paymentId = `sim_${crypto.randomBytes(12).toString("hex")}`;
...
export const createOrder = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        const { receiptUrl } = await generateOrderReceipt({
            orderId:     order._id,
            userName:    user?.name  || "Müştəri",
        ...
        console.error("Çek yaradılarkən xəta:", receiptErr);
    }
