...
const CommissionPage = () => {
  ...
  const sellerId      = user?.sellerInfo?.storeName;
  ...
  useEffect(() => {
    if (transferResult?.receiptUrl) {
      window.open(`http://localhost:3010${transferResult.receiptUrl}`, "_blank");
      if (sellerId) dispatch(getMonthlyCommission({ sellerId, month, year }));
    }
  }, [transferResult]);
  ...
  const handleTransfer = () => {
    ...
    dispatch(doTransferCommission({ sellerId, sellerName, month, year, paymentMethodId }));
  };
  ...
  if (!sellerId) {
    return (
      <div style={s.page}>
        <div style={s.error}>⚠️ Bu səhifəyə yalnız satıcılar daxil ola bilər.</div>
      </div>
    );
  }
  ...
  return (
    <div style={s.page}>
      ...
      {transferResult?.success && (
        <div style={s.success}>✅ {transferResult.message} — PDF çek avtomatik açıldı!</div>
      )}
      {withdrawResult?.success && (
        <div style={s.success}>
          ✅ {withdrawResult.message} — Qalan balans: {withdrawResult.remainingBalance?.toFixed(2)} AZN
        </div>
      )}
      ...
      {monthly && (
        <div style={s.card}>
          <h2 style={s.cardTitle}>{MONTHS[month - 1]} {year} — Xülasə</h2>
          <div style={s.grid}>
            <div style={s.gridItem}>
              <span style={s.gridLabel}>Cəmi Satış</span>
              <span style={s.gridValue}>{monthly.totalOrderAmount?.toFixed(2)} AZN</span>
            </div>
            <div style={s.gridItem}>
            ...
            <div style={s.gridItem}>
              <span style={s.gridLabel}>Sizə Qalan</span>
          ...
          {monthly.pendingAmount > 0 ? (
            <div style={s.transferBox}>
              <p style={{ margin: 0, fontSize: "15px" }}>
                <strong>{monthly.pendingAmount?.toFixed(2)} AZN</strong> komisya köçürülməyi gözləyir
              </p>
              <button
                style={{ ...s.transferBtn, opacity: loading ? 0.6 : 1, cursor: loading ? "not-allowed" : "pointer" }}
            ...
            <div style={s.allDone}>✅ Bu ayın bütün komisyaları köçürülüb</div>
          )}
        </div>
      )}
              ...
              <tbody>
                {monthly.commissions.map((c) => (
                  <tr key={c._id} style={{ borderBottom: "1px solid #f0f0f0" }}>
                    <td style={s.td}>{new Date(c.createdAt).toLocaleDateString("az-AZ")}</td>
                    <td style={s.td}>{c.orderAmount?.toFixed(2)} AZN</td>
                    <td style={{ ...s.td, color: "#E8192C" }}>-{c.commissionAmount?.toFixed(2)} AZN</td>
                    <td style={{ ...s.td, color: "#27ae60" }}>{c.sellerEarning?.toFixed(2)} AZN</td>
                    <td style={s.td}>
                      <span style={{
                        ...s.badge,
                        background: c.status === "transferred" ? "#27ae60" : "#f39c12",
                      }}>
      ...
      {monthly && monthly.totalOrders === 0 && (
        <div style={s.empty}>📭 {MONTHS[month - 1]} {year} üçün sifariş tapılmadı</div>
      )}
      ...
      <div style={s.card}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          />
          <button
            style={{ ...s.withdrawBtn, opacity: loading ? 0.6 : 1, cursor: loading ? "not-allowed" : "pointer" }}
